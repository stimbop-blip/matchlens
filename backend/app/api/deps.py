...
def require_admin_or_support(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> User:
    db_user = db.scalar(select(User).where(User.id == current_user.id))
    if not db_user or db_user.role not in {UserRole.admin, UserRole.support}:
